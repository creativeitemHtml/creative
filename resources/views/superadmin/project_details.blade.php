...
<div class="row box-shadow-6">
    ...
    <div class="col-sm-6 project-items">
        ...
        <div class="project-item mb-20">
            ...
            <!-- File Upload -->
            <form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('upload.project_attachment', ['project_id' => $project_details->id]) }}">
                @csrf
                ...
                <tbody>
                    @foreach($attachments as $key => $attachment)
                        <tr>
                            <td>
                                <div class="min-w">
                                    <p class="fz-15-sb-black">{{ ($key+1).'. ' }}<a href="{{ route('download.project_attachment', ['project_id' => $project_details->id, 'key' => $key]) }}">{{ $attachment }}</a></p>
                                </div>
                            </td>
                            <td>
                                <div class="min-w">
                                    ...
                                    <div class="payfile-option">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li>
                                                <a class="dropdown-item" href="{{ route('download.project_attachment', ['project_id' => $project_details->id, 'key' => $key]) }}">{{ get_phrase('Download') }}</a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" href="javascript:;" onclick="confirmModal('{{ route('remove.project_attachment', ['project_id' => $project_details->id, 'key' => $key]) }}', 'undefined');">{{ get_phrase('Delete') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
