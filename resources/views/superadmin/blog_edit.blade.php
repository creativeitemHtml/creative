<div class="admin_main_right p-30 bd-r-5">
    <div class="d-flex justify-content-between align-items-center flex-wrap g-10 pb-30">
        ...
        <button class="d-lg-none mobile-menu-2" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">
            <img src="{{ asset('public/assets/img/new-icons-images/menu-icon.svg') }}" alt="menu">
        </button>
    </div>

    <!-- Form -->
    <form id="blog_form" action="{{ route('superadmin.edit_blog', ['id' => $blog_details->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="dl_column_form">
            <div class="nproject-form-wrap">
                <div class="pForm-wrap">
                    <label for="blog_category_id" class="enForm-label">{{ get_phrase('Blog Category') }}</label>
                    ...
                    @foreach ($blog_categories as $key => $blog_category)
                        <option value="{{ $blog_category->id }}" @if($blog_category->id == $blog_details->blog_category_id) selected @endif>{{ $blog_category->name }}</option>
                    @endforeach
                </select>
                </div>
                <div class="pForm-wrap">
                ...
                <div class="pForm-wrap">
                    ...
                    <input class="form-control tag" type="text" id="tags" name="tags" placeholder="Put comma(,) after each word" value="{{ $blog_details->tags }}">
                    @if ($errors->has('tags'))
                ...
                <div class="pForm-wrap">
                    ...
                    <input type="text" class="form-control enForm-control" id="excerpt" name="excerpt" placeholder="Excerpt" aria-label="Excerpt" value="{{ $blog_details->excerpt }}" />
                </div>
                <div class="pForm-wrap">
                    <label for="read_time" class="enForm-label">{{ get_phrase('Read Time') }}</label>
                    <input type="text" class="form-control enForm-control" id="read_time" name="read_time" placeholder="Ex: 13" aria-label="read_time" value="{{ $blog_details->read_time }}" />
                </div>
                <!-- Editor -->
                <div class="pForm-wrap editor-wrap">
                    <label for="blog_details" class="enForm-label">{{ get_phrase('Blog') }}</label>
                    <textarea id="mytextarea" name="blog_details" placeholder="About your project">{{ $blog_details->details }}</textarea>
                </div>
                <div class="pForm-wrap">
                ...
                <ul class="productPriceOption">
                    <li>
                        <div class="form-check">
                            <input class="form-check-input ciRadio ciRadio-OutlinePrimary" type="checkbox" name="visibility" id="visibility" @if($blog_details->visibility == 1) checked @endif />
                            <label class="form-check-label" for="visibility">{{ get_phrase('Make this blog public') }}</label>
                        </div>
                    </li>
                    <li>
                        <div class="form-check">
                            <input class="form-check-input ciRadio ciRadio-OutlinePrimary" type="checkbox" name="ability_to_comment" id="ability_to_comment" @if($blog_details->ability_to_comment == 1) checked @endif />
                            <label class="form-check-label" for="ability_to_comment">{{ get_phrase('People can leave their comment') }}</label>
                        </div>
                    </li>
                    <li>
                        <div class="form-check">
                            <input class="form-check-input ciRadio ciRadio-OutlinePrimary" type="checkbox" name="is_featured" id="is_featured" @if($blog_details->is_featured == 1) checked @endif />
                ...
                <div class="pForm-wrap">
                    ...
                    <span id="count_meta_title">({{ strlen($blog_details->meta_title ?? '') }} charecters)</span>
                    <input type="text" class="form-control enForm-control" name="meta_title" id="meta_title" placeholder="Blog Title" aria-label="Blog Title" value="{{ $blog_details->meta_title }}" onkeyup="count_character('meta_title')" />
                </div>
                ...
                <div class="pForm-wrap">
                    ...
                    <input type="text" class="form-control enForm-control" name="meta_keywords" id="meta_keywords" placeholder="Meta Keywords" aria-label="Meta Keywords" value="{{ $blog_details->meta_keywords }}" onkeyup="count_character('meta_keywords')" />
                </div>

                <div class="pForm-wrap">
                    <label for="meta_title" class="enForm-label">{{ get_phrase('Meta Description') }}</label>
                    <span id="count_meta_description">({{ strlen($blog_details->meta_description ?? '') }} charecters)</span>
                    <input type="text" class="form-control enForm-control" name="meta_description" id="meta_description" placeholder="Meta Description" aria-label="Meta Description" value="{{ $blog_details->meta_description }}" onkeyup="count_character('meta_description')" />
                </div>

                <div class="pForm-wrap">
                    <label for="custom_url" class="enForm-label">{{ get_phrase('Custom Url') }}</label>
                    <input type="text" class="form-control enForm-control" palaceholder="https://example.com" name="custom_url" value="{{ $blog_details->custom_url }}" />
                </div>

                <div class="pForm-wrap">
                    <label for="canonical_url" class="enForm-label">{{ get_phrase('Canonical Url') }}</label>
                    <input type="text" class="form-control enForm-control" palaceholder="https://example.com" name="canonical_url" value="{{ $blog_details->canonical_url }}" />
                </div>

                <div class="pForm-wrap">
                    <label for="meta_title" class="enForm-label">{{ get_phrase('Og Title') }}</label>
                    <span id="count_og_title">({{ strlen($blog_details->og_title ?? '') }} charecters)</span>
                    <input type="text" class="form-control enForm-control" name="og_title" id="og_title" value="{{ $blog_details->og_title }}" onkeyup="count_character('og_title')" />
                </div>

                <div class="pForm-wrap">
                    <label for="og_description" class="enForm-label">{{ get_phrase('Og Description') }}</label>
                    <span id="count_og_description">({{ strlen($blog_details->og_description ?? '') }} charecters)</span>
                    <input type="text" class="form-control" name="og_description" id="og_description" value="{{ $blog_details->og_description }}" onkeyup="count_character('og_description')" />
                </div>
                ...
                <div class="pForm-wrap">
                    ...
                    <textarea class="form-control enForm-control" name="json_ld" id="json_ld" cols="10" rows="4">{{ $blog_details->json_ld }}</textarea>
                </div>
            </div>
        </div>
        <!-- Button -->
