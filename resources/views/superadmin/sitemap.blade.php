...
<div class="admin_main_right p-30 bd-r-5 mb-60">
    ...
    <!-- Form -->
    <form id="sitemap_form" action="{{ route('sitemap_settings') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="dl_column_form">
            <div class="cForm-wrap">
                <div class="enForm-label">
                    <label for="{{ $sitemap->key }}" class="eForm-label">{{ get_phrase('Sitemap xml') }}</label>
                    <textarea class="form-control enForm-control" name="{{ $sitemap->key }}" id="{{ $sitemap->key }}" cols="5" rows="25">{{ $sitemap->value  }}</textarea>
                </div>
            </div>
        </div>
        <!-- Button -->
